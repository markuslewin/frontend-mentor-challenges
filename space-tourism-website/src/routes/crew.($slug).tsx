  ...
  return (
    <div className="mt-8 tablet:mt-[3.75rem] desktop:mt-0 mx-auto max-w-[74.25rem] px-6 tablet:px-10 pb-28 tablet:pb-0 grid desktop:grid-cols-2">
      <div className="order-3 tablet:order-1 mt-8 tablet:mt-0 mx-auto desktop:mx-0 max-w-[28.625rem] desktop:max-w-none text-center desktop:text-start">
        ...
        <p className="mt-4 desktop:mt-7">{member.bio}</p>
      </div>
      <picture className="order-1 tablet:order-3 desktop:col-start-2 desktop:row-start-1 desktop:row-span-2 tablet:mt-10 desktop:mt-0 border-b-[1px] border-[hsl(231,15%,26%)] tablet:border-b-0">
      ...
      <nav
        className="order-2 tablet:order-2 desktop:col-start-1 mt-8 tablet:mt-10 desktop:mt-[7.5rem]"
...
function CrewNavLink(props: NavLinkProps) {
  return (
    <NavLink
      className={({ isActive }) =>
        `dot ${isActive ? "text-FFFFFF" : "text-FFFFFF/[17.44%]"}`
      }
      {...props}
    />
  );
